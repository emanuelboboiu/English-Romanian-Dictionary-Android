...
public class SearchHistoryActivity extends Activity implements OnItemSelectedListener {
    ...
    private SpeakText speak; // for English.
    private SpeakText2 speak2; // for Romanian.
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == R.id.cmSearchResult) {
            ...
            return true;
        } else if (item.getItemId() == R.id.cmDeleteResult) {
    ...
    // A method which speaks the word depending of the language:
    private void speakWord(String w) {
        if (direction == 0) {
            // Only for English words:
            speak.sayUsingLanguage(w, false);
        } else { // Romanian:
            speak2.sayUsingLanguage(w, false);
        } // end if Romanian results.
    } // end speakWord() method.
