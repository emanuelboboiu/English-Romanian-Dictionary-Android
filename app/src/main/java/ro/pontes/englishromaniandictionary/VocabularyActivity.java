...
public class VocabularyActivity extends Activity implements OnItemSelectedListener {
    ...
    private void createList(String categoryName) {
        ...
        do {
            ...
            String tvText = String.format(toFormat, curWord, MainActivity.weSeparator, curExplanation);
            ...
            tv.setOnClickListener(view -> speak.sayUsingLanguage(englishPart, true));
...
// end Google ads section.
    ...
    private void renameCategory() {
        ...
        if (!curCategoryName.equals("%")) {
            ...
            tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, MainActivity.textSize);
            String message = MyHtml.fromHtml(String.format(getString(R.string.message_rename_section), curCategoryName)).toString();
            tv.setText(message);
            tv.setFocusable(true);
            ll.addView(tv, etlp);
            ...
            etName.setInputType(InputType.TYPE_TEXT_FLAG_CAP_SENTENCES | EditorInfo.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
            ...
            AlertDialog.Builder alert = new AlertDialog.Builder(context).setTitle(tempTitle).setView(ll).setPositiveButton(R.string.rename, (dialog, whichButton) -> {
                String newName = etName.getText().toString();
    ...
    private boolean saveRenameSection(String name) {
        ...
        if (newname.length() >= 2) {
            ...
            if (!fieldExists("sectiuni", "nume", newname)) {
                ...
                GUITools.alert(mFinalContext, getString(R.string.warning), getString(R.string.this_vocabulary_section_already_exists_rename));
            } // end if section name already exist.
        } // end if new name edit has text.
        else {
            GUITools.alert(mFinalContext, getString(R.string.warning), getString(R.string.no_texts_for_new_section_name));
    ...
    private void addRecord() {
        ...
        if (!curCategoryName.equals("%")) {
            ...
            // end if save button was pressed.
            AlertDialog.Builder alert = new AlertDialog.Builder(context).setTitle(tempTitle).setView(ll).setPositiveButton(R.string.save, (dialog, whichButton) -> {
                String newWord = etWord.getText().toString();
                String newExplanation = etExplanation.getText().toString();
    ...
    private void importPredefinedCategories() {
        ...
        try {
            ...
            // Create the alert:
            AlertDialog.Builder alert = new AlertDialog.Builder(context).setTitle(getString(R.string.title_import_predefined)).setView(sv).setNegativeButton(android.R.string.cancel, null);
            alertToShow = alert.create();
            alertToShow.show();
            // end try.
        } catch (IOException e) {
